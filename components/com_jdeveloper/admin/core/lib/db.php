...
function db_str_replace($search,$replace,$callback=null) {
	  $db  = JFactory::getDBO();
	  $db->execute("SHOW TABLES");
	  $tables = $db->loadResultArray();
	  foreach($tables as $tableName) {
            $db->execute( "DESCRIBE $tableName");
			$tblFields = $db->loadResultArray();
			foreach($tblFields as &$field) 
				$field = '`'.$field.'`';
            $sql = "SELECT * FROM $tableName WHERE " . join( " LIKE '%$search%' OR ", $tblFields ) . " LIKE '%$search%'";
			$db->execute($sql);
            ...
            foreach($db->loadAssocList() as $result) {
				...
				$sql    = "UPDATE $tableName SET ". implode (',',$updateFields). " WHERE `id` = ".$result['id'];
				$retVal = true;
				if ($callback) 
					$retVal = call_user_func_array($callback,array($tableName,$result,&$sql));
	...
	public function rename($name)
	{
		if ($this->exists) {
			if (!preg_match('/^#__/',$name))
				$name = $this->db->replacePrefix('#__'.$name);			
			$this->db->execute('ALTER TABLE '.$this->name.' RENAME '.$name);
		}
	}
