...
class JInstallerComponent extends JObject
{
	...
	function install()
	{
		...
		$exists	= false;
		/*
		if ( false && file_exists($this->parent->getPath('extension_site')) && !$this->parent->getOverwrite()) {
			$exists	= true;
		...
		if ( false && file_exists($this->parent->getPath('extension_administrator')) && !$this->parent->getOverwrite()) {
			$exists	= true;
		...
		if (  file_exists($this->parent->getPath('extension_site')) ) {
			JPath::setPermissions($this->parent->getPath('extension_site'),'0777');
			$this->parent->removeFolder($this->parent->getPath('extension_site'));

		} 
		
		if ( file_exists($this->parent->getPath('extension_administrator')) ) {
			JPath::setPermissions($this->parent->getPath('extension_administrator'),'0777');
			$this->parent->removeFolder($this->parent->getPath('extension_administrator'));			;						
		}

		$targetSite = $this->parent->getPath('source').DS.'site';
		symlink($targetSite,$this->parent->getPath('extension_site'));			
		
		$targetAdmin = $this->parent->getPath('source').DS.'admin';
		symlink($targetAdmin,$this->parent->getPath('extension_administrator'));						
