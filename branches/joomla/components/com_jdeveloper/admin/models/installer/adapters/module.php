...
/*
	...
	{
		//find all the instances of this module
		$db		=& $this->parent->getDBO();
		$q = "SELECT id FROM #__modules WHERE module LIKE 'mod_$id' AND client_id = $clientId";
		$db->execute($q);		
		$modIds = $db->loadResultArray();
		if ( count($modIds) ) {
			$modIds = implode(',',$modIds);
			$q = 'DELETE' .
					' FROM #__modules_menu' .
					' WHERE moduleid IN ('.$modIds.')';
		...
		if (!$client) {
			JError::raiseWarning(100, JText::_('Module').' '.JText::_('Uninstall').': '.JText::_('Invalid application'));
			return false;
		}
		$this->parent->setPath('extension_root', $client->path.DS.'modules'.DS.'mod_'.$id);
